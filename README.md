...
19. `--browser` only accepts `chrome`, `chromium`, `firefox`, or `webkit` - an unrecognised value (e.g. a typo) is refused with a clear "Unsupported browser type" error before ever reaching Playwright, instead of an opaque native error. Matching is case-insensitive - `--browser Chrome` or `--browser WEBKIT` resolve the same as their lowercase forms (D2B-185).
