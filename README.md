...
`.github/workflows/cross-platform.yml` runs a real (non-mocked) Playwright browser launch on every push/pull request, across `ubuntu-latest`, `ubuntu-24.04-arm`, and `macos-latest` for each of `chrome`/`chromium`/`edge`/`firefox`/`webkit` - the only place a genuine launch is exercised, since every `t/*.t` test in this repo mocks Playwright instead. The chrome-install step substitutes `chromium` for `ubuntu-24.04-arm` specifically, since Playwright's own installer refuses the branded `chrome` channel entirely on Linux ARM64 - the runtime `--browser chrome` flag is unaffected, since this skill's `chrome` browser type has no `executablePath` set on a fresh runner anyway and already launches Playwright's bundled chromium build either way (D2B-199). `edge` is excluded from the `ubuntu-24.04-arm` combination entirely, since Microsoft ships no Edge build for Linux ARM64 at all - unlike chrome, there is no substitute install target that would actually exercise edge there. Playwright's own install CLI names the Edge channel `msedge`, not `edge`, so the install step maps this skill's `--browser edge` value to that install target name specifically (D2B-200). `windows-latest` is excluded entirely (D2B-203): D2B-202 first fixed a Perl XS ABI mismatch there (`distribution: strawberry`, working around a known, open bug in `shogo82148/actions-setup-perl`, issue #2310, kept in the workflow as dormant documentation for if Windows is ever restored) - but that exposed a second, deeper issue underneath it: the CPAN `Playwright` module's own Windows support is documented upstream as an experimental, separate implementation (`teodesian/playwright-perl#22`) that fails to spawn its Node driver reliably on this runner for every browser identically, a genuine upstream limitation rather than something to patch inside this skill's own repo. `macos-13` is also excluded entirely (D2B-204): its jobs queued with zero progress for 4+ hours across 5 separate real runs, a GitHub-hosted macOS Intel runner capacity issue on this account - `macos-latest` (Apple Silicon) always completed within minutes on those same runs, so macOS coverage continues through that entry alone.
