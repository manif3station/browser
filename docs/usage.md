...
- a URL argument is only refused as missing when it is truly absent or an empty string - a URL that happens to be the single character `0` is accepted and used as-is, not rejected by Perl truthiness
- `--browser` only accepts `chrome`, `chromium`, `firefox`, or `webkit` - an unrecognised value is refused with a clear "Unsupported browser type" error before ever reaching Playwright
- `--timeout-ms` is refused with "--timeout-ms must not be negative" on `browser.get`/`browser.post`/`browser.png`, matching `browser.search`'s own negativity guard on its own `--timeout-ms`
