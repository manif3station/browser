...
- `--browser` only accepts `chrome`, `chromium`, `firefox`, or `webkit` - an unrecognised value is refused with a clear "Unsupported browser type" error before ever reaching Playwright. Matching is case-insensitive - `--browser Chrome` or `--browser WEBKIT` resolve the same as their lowercase forms (D2B-185).
